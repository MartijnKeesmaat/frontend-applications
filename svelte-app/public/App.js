...
const RetrieveData = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	return `${ results.length > 0 ? `<h2>${escape(results[0].title.value)}</h2>
	  <p>${escape(results[0].description.value)}</p>
	  <img${add_attribute("src", results[0].imageLink.value, 0)} alt="">` : `<p>Loading</p>` }`;
});
