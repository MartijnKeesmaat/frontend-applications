...
const RetrieveData = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	return `${ results.length > 0 ? `<h2>${escape(results[0].title)}</h2>
	  <p>${escape(results[0].description)}</p>
	  <img${add_attribute("src", results[0].imageLink, 0)} alt="">

	  ${each(results, ({ id, title, description, imageLink }, i) => `<h2>${escape(title)}</h2>
